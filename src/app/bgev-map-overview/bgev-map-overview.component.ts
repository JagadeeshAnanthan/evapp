...
constructor(private _bottomSheetRef: MatBottomSheetRef<BgEvMapOverviewComponent>,
    ...
    redefineSearch() {
        ...
        if (!this.location) {
            ...
            this._bottomSheetRef.dismiss([this.latitude, this.longitude, this.postcode]);
        }
    }
    ...
    async getAddress(value) {
        if (value.length > 3) {
            ...
            this.location = response[0].Location.Address.Label;
            this.position = response[0].Location.DisplayPosition;
            this.postcode = response[0].Location.Address.PostalCode;
            console.log(this.position);
            this.setLatLongitude(this.position, this.postcode);
        } 

    }

    setLatLongitude(position, postCode) {
